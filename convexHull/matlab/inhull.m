...
if (nargin<3) || isempty(tess)
  tess = convhulln(xyz);
  %disp('tess is as follows:');
  %disp(tess);
  disp(size(tess,1));
    %tess = convhulln(xyz,{'QJ','QbB','Pp','Qt'});%yuroc
  %tess = convexHull(xyz);%yuroc
end
[nt,c] = size(tess);
if c ~= p
...
switch p
  case 2
   ...
   % disp('2D normal vectors are as follows:');
   % disp(nrmls);
...
%disp('scale normal vectors as follows:');
%disp(nrmls);
...
%disp('repeat copies of center into dimension ntx1 then minus points on convex hull:');
%disp(repmat(center,nt,1)-a);
%disp('pointwise product with normal vector:');
%disp((repmat(center,nt,1) - a).*nrmls);
k = dp<0;
%disp(nrmls);
nrmls(k,:) = -nrmls(k,:);
%disp('normal vector after sign conversion:');
%disp(nrmls);
% We want to test if:  dot((x - a),N) >= 0
% If so for all faces of the hull, then x is inside
% the hull. Change this to dot(x,N) >= dot(a,N)
%aN is the dot product of normal vector and existing points that are on
%convex hull, w.r.t each row, aka dot(a,N)
aN = sum(nrmls.*a,2);
%disp(a);

%disp(size(nrmls,1));

%testaN=dot(nrmls,a,2);
% disp('aN is');
% disp(aN);
% disp('my aN is');
% disp(testaN);
...
tic
for i=1:n
singletest=repmat(testpts(i,:),nt,1);
dotprod=dot(nrmls,singletest,2);
myin(i)=all(dotprod-aN>=-tol,1)';
end
toc
%disp('myin is');
%disp(myin);
